...
proc rad_bootstrap_cross_environment_pkg_config*() =
  putEnv(RAD_ENV_PKG_CONFIG_LIBDIR, getEnv(RAD_ENV_DIR_CRSD) / RAD_PATH_PKG_CONFIG_LIBDIR_PATH)
...
proc rad_bootstrap_cross_environment_teeth*() =
  ...
  putEnv(RAD_ENV_TOOTH_CPP, cross_compile & RAD_CERAS_GCC & ' ' & RAD_TOOTH_CPP)
  putEnv(RAD_ENV_TOOTH_CXX, cross_compile & RAD_TOOTH_CXX)
  putEnv(RAD_ENV_TOOTH_CXXCPP, cross_compile & RAD_TOOTH_CXX & ' ' & RAD_TOOTH_CPP)
...
proc rad_bootstrap_release_img*() =
  ...
  discard execCmd(&"{RAD_TOOTH_DD} bs=1M count={RAD_FILE_GLAUCUS_IMG_SIZE} if=/dev/zero of={img} {RAD_FLAGS_TOOTH_SHELL_REDIRECTION}")

  # Partition the IMG file
  discard execCmd(&"{RAD_TOOTH_PARTED} {RAD_FLAGS_TOOTH_PARTED} {img} mklabel msdos {RAD_FLAGS_TOOTH_SHELL_REDIRECTION}")
  discard execCmd(&"{RAD_TOOTH_PARTED} {RAD_FLAGS_TOOTH_PARTED} -a none {img} mkpart primary ext4 1 {RAD_FILE_GLAUCUS_IMG_SIZE} {RAD_FLAGS_TOOTH_SHELL_REDIRECTION}")
  discard execCmd(&"{RAD_TOOTH_PARTED} {RAD_FLAGS_TOOTH_PARTED} -a none {img} set 1 boot on {RAD_FLAGS_TOOTH_SHELL_REDIRECTION}")

  # Load the `loop` module
  discard execCmd(&"{RAD_TOOTH_MODPROBE} loop {RAD_FLAGS_TOOTH_SHELL_REDIRECTION}")

  # Detach all used loop devices
  discard execCmd(&"{RAD_TOOTH_LOSETUP} -D {RAD_FLAGS_TOOTH_SHELL_REDIRECTION}")
  ...
  discard execCmd(&"{RAD_TOOTH_LOSETUP} {device} {img} {RAD_FLAGS_TOOTH_SHELL_REDIRECTION}")

  # Notify the kernel about the new partition on the IMG file
  discard execCmd(&"{RAD_TOOTH_PARTX} -a {device} {RAD_FLAGS_TOOTH_SHELL_REDIRECTION}")
  ...
  discard execCmd(&"{RAD_TOOTH_MOUNT} {partition} {mount} {RAD_FLAGS_TOOTH_SHELL_REDIRECTION}")
  ...
  discard execCmd(&"{RAD_TOOTH_GRUB_INSTALL} {RAD_FLAGS_TOOTH_GRUB} --boot-directory={mount / RAD_PATH_BOOT} --target=i386-pc {device} {RAD_FLAGS_TOOTH_SHELL_REDIRECTION}")

  # Change ownerships
  discard execCmd(&"{RAD_TOOTH_CHOWN} {RAD_FLAGS_TOOTH_CHOWN} 0:0 {mount} {RAD_FLAGS_TOOTH_SHELL_REDIRECTION}")
  discard execCmd(&"{RAD_TOOTH_CHOWN} {RAD_FLAGS_TOOTH_CHOWN} 20:20 {mount / RAD_PATH_VAR / RAD_DIR_LOG / RAD_PATH_WTMPD} {RAD_FLAGS_TOOTH_SHELL_REDIRECTION}")

  # Clean up
  discard execCmd(&"{RAD_TOOTH_UMOUNT} {RAD_FLAGS_TOOTH_UMOUNT} {mount} {RAD_FLAGS_TOOTH_SHELL_REDIRECTION}")
  discard execCmd(&"{RAD_TOOTH_PARTX} -d {partition} {RAD_FLAGS_TOOTH_SHELL_REDIRECTION}")
  discard execCmd(&"{RAD_TOOTH_LOSETUP} -d {device} {RAD_FLAGS_TOOTH_SHELL_REDIRECTION}")
...
proc rad_bootstrap_release_iso*() =
  ...
  # Compress rootfs
  discard execCmd(&"{RAD_TOOTH_MKFS_EROFS} {path / RAD_FILE_ROOTFS} {getEnv(RAD_ENV_DIR_CRSD)} {RAD_FLAGS_TOOTH_SHELL_REDIRECTION}")

  # Copy kernel
  discard rad_rsync(getEnv(RAD_ENV_DIR_CRSD) / RAD_PATH_BOOT / RAD_FILE_KERNEL, path)

  # Create a new ISO file
  discard execCmd(&"{RAD_TOOTH_GRUB_MKRESCUE} {RAD_FLAGS_TOOTH_GRUB} -v -o {iso} {getEnv(RAD_ENV_DIR_ISOD)} -volid GLAUCUS {RAD_FLAGS_TOOTH_SHELL_REDIRECTION}")
...
proc rad_bootstrap_system_environment_pkg_config*() =
  putEnv(RAD_ENV_PKG_CONFIG_LIBDIR, RAD_PATH_PKG_CONFIG_LIBDIR_PATH)
  putEnv(RAD_ENV_PKG_CONFIG_PATH, getEnv(RAD_ENV_PKG_CONFIG_LIBDIR))
...
proc rad_bootstrap_system_environment_teeth*() =
  ...
  putEnv(RAD_ENV_TOOTH_CPP, RAD_CERAS_GCC & ' ' & RAD_TOOTH_CPP)
  putEnv(RAD_ENV_TOOTH_CXX, RAD_TOOTH_CXX)
  putEnv(RAD_ENV_TOOTH_CXXCPP, RAD_TOOTH_CXX & ' ' & RAD_TOOTH_CPP)
  putEnv(RAD_ENV_TOOTH_HOSTCC, RAD_CERAS_GCC)
