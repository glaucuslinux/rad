...
proc releaseIso*() =
  ...
  discard execCmd(&"{grubMkrescue} {Grub} -o {iso} --product-name {glaucus} {getEnv($ISOD)} -A {glaucus} -iso-level 3 -J -joliet-long -l -pad -publisher {glaucus} -p {glaucus} -R -r -V {toUpperAscii($glaucus)} -vv {shellRedirect}")
