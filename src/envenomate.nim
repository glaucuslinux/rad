...
proc radula_behave_stage*(nom, ver, stage = RADULA_DIRECTORY_SYSTEM, log: string): int =
    ...
    execCmd(&"{RADULA_CERAS_DASH} {RADULA_TOOTH_SHELL_COMMAND_FLAGS} 'nom={nom} ver={ver} . {RADULA_PATH_RADULA_CLUSTERS}/{RADULA_DIRECTORY_GLAUCUS}/{nom}/{stage} && prepare $1 && configure $1 && build $1 && check $1 && install $1'" % [&">> {log} {RADULA_TOOTH_SHELL_REDIRECTION}"])
