...
proc radula_ceras_stage*(log, nom, ver: string, stage = RADULA_DIRECTORY_SYSTEM): int =
  ...
  execCmd(&"nom={nom} ver={ver} {RADULA_CERAS_ZSH} {RADULA_TOOTH_SHELL_COMMAND_FLAGS} '. {RADULA_PATH_RADULA_CLUSTERS_GLAUCUS}/{nom}/{stage} && ceras_prepare $1 && ceras_configure $1 && ceras_build $1 && ceras_check $1 && ceras_install $1'" % [&">> {log} 2>&1"])
